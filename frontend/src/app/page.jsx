'use client';

import React from 'react'
import Navbar from './navbar';
import { SlCalender } from "react-icons/sl";
import { TfiLocationPin } from "react-icons/tfi";
import { FaRegClock } from "react-icons/fa6";
import { GiTakeMyMoney } from "react-icons/gi";


const Home = () => {
  return (

    <div>
      <Navbar />
      <>
        {/* Slider */}
        <div
          data-hs-carousel='{
    "loadingClasses": "opacity-0",
    "isAutoPlay": true
  }'
          className="relative"
        >
          <div className="hs-carousel relative overflow-hidden w-full h-svh bg-white rounded-lg">
            <div className="hs-carousel-body absolute top-0 bottom-0 start-0 flex flex-nowrap transition-transform duration-700 opacity-0">
              <div className="hs-carousel-slide">
                <div className="flex justify-center h-full bg-gray-100 p-6 dark:bg-neutral-900">
                  <span className="self-center text-4xl text-gray-800 transition duration-700 dark:text-white">
                  <img className='' src="https://mcn-images.bauersecure.com/wp-images/4658/1440x960/honda-cb300r-01.jpg?mode=max&quality=90&scale=down" alt="" />
                  </span>
                </div>
              </div>
              <div className="hs-carousel-slide">
                <div className="flex justify-center h-full bg-gray-200 p-6 dark:bg-neutral-800">
                  <span className="self-center text-4xl text-gray-800 transition duration-700 dark:text-white">
                  <img src="https://static1.hotcarsimages.com/wordpress/wp-content/uploads/2021/02/2019-Yamaha-V-Star-650-XVS650-MJK9687.jpg" alt="" />
                  </span>
                </div>
              </div>
              <div className="hs-carousel-slide">
                <div className="flex justify-center h-full bg-gray-300 p-6 dark:bg-neutral-700">
                  <span className="self-center text-4xl text-gray-800 transition duration-700 dark:text-white">
                  <img src="https://media-blog.zutobi.com/wp-content/uploads/sites/2/2022/03/11152138/balance.jpeg?w=2560&auto=format&ixlib=next&fit=max" alt="" />
                  </span>
                </div>
              </div>
            </div>
          </div>
          <button
            type="button"
            className="hs-carousel-prev hs-carousel:disabled:opacity-50 disabled:pointer-events-none absolute inset-y-0 start-0 inline-flex justify-center items-center w-[46px] h-full text-gray-800 hover:bg-gray-800/10 rounded-s-lg dark:text-white dark:hover:bg-white/10"
          >
            <span className="text-2xl" aria-hidden="true">
              <svg
                className="flex-shrink-0 size-5"
                xmlns="http://www.w3.org/2000/svg"
                width={24}
                height={24}
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth={2}
                strokeLinecap="round"
                strokeLinejoin="round"
              >
                <path d="m15 18-6-6 6-6" />
              </svg>
            </span>
            <span className="sr-only">Previous</span>
          </button>
          <button
            type="button"
            className="hs-carousel-next hs-carousel:disabled:opacity-50 disabled:pointer-events-none absolute inset-y-0 end-0 inline-flex justify-center items-center w-[46px] h-full text-gray-800 hover:bg-gray-800/10 rounded-e-lg dark:text-white dark:hover:bg-white/10"
          >
            <span className="sr-only">Next</span>
            <span className="text-2xl" aria-hidden="true">
              <svg
                className="flex-shrink-0 size-5"
                xmlns="http://www.w3.org/2000/svg"
                width={24}
                height={24}
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth={2}
                strokeLinecap="round"
                strokeLinejoin="round"
              >
                <path d="m9 18 6-6-6-6" />
              </svg>
            </span>
          </button>
          <div className="hs-carousel-pagination flex justify-center absolute bottom-3 start-0 end-0 space-x-2">
            <span className="hs-carousel-active:bg-blue-700 hs-carousel-active:border-blue-700 size-3 border border-gray-400 rounded-full cursor-pointer dark:border-neutral-600 dark:hs-carousel-active:bg-blue-500 dark:hs-carousel-active:border-blue-500" />
            <span className="hs-carousel-active:bg-blue-700 hs-carousel-active:border-blue-700 size-3 border border-gray-400 rounded-full cursor-pointer dark:border-neutral-600 dark:hs-carousel-active:bg-blue-500 dark:hs-carousel-active:border-blue-500" />
            <span className="hs-carousel-active:bg-blue-700 hs-carousel-active:border-blue-700 size-3 border border-gray-400 rounded-full cursor-pointer dark:border-neutral-600 dark:hs-carousel-active:bg-blue-500 dark:hs-carousel-active:border-blue-500" />
          </div>
        </div>
        {/* End Slider */}
      </>

      <div className="container mx-auto">
        <h1 style={{fontFamily:"tstar-medium"}} className='text-center my-10 text-4xl font-bold'>UNBOUND NORTH EAST</h1>
        <span className='text-xl' style={{fontFamily:"tstar-medium"}}>Embark on an exhilarating journey and get ready to traverse breath-taking landscapes, winding mountain roads, and vibrant cultures as you unbound the untold stories. Join us as we ride through the adventurous territories of the land of the rising sun in the Northeast, forging unforgettable memories and bonds that transcend borders.</span>
      </div>

       <div className="container">
        <div className="grid grid-cols-4 gap-3 ">
          <div className=''>
          <SlCalender className='size-10 ' />
          </div>
          <div>
          <TfiLocationPin />
          </div>
          <div>
          <FaRegClock />
          </div>
          <div>
          <GiTakeMyMoney />
          </div>
        </div>
       </div>


    </div>
  )
}

export default Home